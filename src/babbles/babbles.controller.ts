...
export class BabblesController {
  ...
  @Get()
  @ApiOperation({ summary: '대화방 목록', description: '요청자의 대화방 목록이 반환.(bearer token 필수)' })
  ...
  @Post()
  @ApiOperation({ summary: '대화방 생성', description: '부여된 성격으로 대화 상대를 설정 하여 방을 생성.(bearer token 필수)' })
  ...
  @Post(':id')
  @ApiOperation({ summary: '대화하기', description: '요청자의 메세지를 처리하여 AI의 메세지를 응답으로 반환.(bearer token 필수)' })
  ...
  @Get(':id')
  @ApiOperation({ summary: '대화 이력', description: '현재 대화방의 모든 대화 내용을 응답으로 반환 (bearer token 필수)' })
