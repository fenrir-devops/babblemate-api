...
export class UsersController {
  ...
  @Post('/new')
  @ApiOperation({ summary: '신규 유저 등록', description: '전송된 데이터를 토대로 신규 유저 정보를 등록한다.' })
  @ApiResponse({ status: 201 }) 
  @ApiResponse({ status: 400, description: '부정확한 정보를 포함하고 있거나 기타 장애로 생성 실패시' }) 
  ...
  @Get('/auth')
  @ApiOperation({ summary: '유저 확인', description: '전송된 데이터를 토대로 유저 정보를 확인하여 억세스토큰을 발급한다.' })
